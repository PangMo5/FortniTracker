...
class UserDetailViewModel: NSObject, ViewModelType {
    ...
    func transform(input: Input) -> Output {
        ...
        input.refresh.flatMapLatest({ [weak self] () -> Observable<UserDetail> in
            ...
            return self.provider.rx
                ...
                .asObservable()
                .catchErrorJustReturn(UserDetail())
        }).subscribe(onNext: { item in
            
            var sectionElements = [UserDetailSection]()
            //total
            if let totalData = item.overallData {
                let totalKD = UserDetailSectionItem.kd(viewModel: UserDetailCellViewModel(with: "K/D", detailTitle: self.kd(kill: totalData.kills, wins: totalData.placetop1, matchs: totalData.matchesplayed)))
            ...
            if let soloData = item.data?.defaultsolo {
                let soloKD = UserDetailSectionItem.kd(viewModel: UserDetailCellViewModel(with: "K/D", detailTitle: self.kd(kill: soloData.kills, wins: soloData.placetop1, matchs: soloData.matchesplayed)))
                ...
                sectionElements.append(UserDetailSection.solo(title: "Solo\nto \(soloData.lastmodified?.dateString() ?? "")", detailTitle: "\(soloData.matchesplayed ?? 0) matches\n\(soloData.minutesplayed ?? 0) minutes", items: [soloKD, soloKills, soloPlacetop1, soloPlacetop2, soloPlacetop3, soloScore]))
            }
            
            //duo
            
            if let duoData = item.data?.defaultduo {
                let duoKD = UserDetailSectionItem.kd(viewModel: UserDetailCellViewModel(with: "K/D", detailTitle: self.kd(kill: duoData.kills, wins: duoData.placetop1, matchs: duoData.matchesplayed)))
                ...
                sectionElements.append(UserDetailSection.duo(title: "Duo\nto \(duoData.lastmodified?.dateString() ?? "")", detailTitle: "\(duoData.matchesplayed ?? 0) matches\n\(duoData.minutesplayed ?? 0) minutes", items: [duoKD, duoKills, duoPlacetop1, duoPlacetop2, duoPlacetop3, duoScore]))
            }
            
            //squad
            
            if let squadData = item.data?.defaultsquad {
                let squadKD = UserDetailSectionItem.kd(viewModel: UserDetailCellViewModel(with: "K/D", detailTitle: self.kd(kill: squadData.kills, wins: squadData.placetop1, matchs: squadData.matchesplayed)))
                ...
                sectionElements.append(UserDetailSection.squad(title: "Squad\nto \(squadData.lastmodified?.dateString() ?? "")", detailTitle: "\(squadData.matchesplayed ?? 0) matches\n\(squadData.minutesplayed ?? 0) minutes", items: [squadKD, squadKills, squadPlacetop1, squadPlacetop2, squadPlacetop3, squadScore]))
            }
    ...
    fileprivate func kd(kill: Int?, wins: Int?, matchs: Int?) -> String {
        return String(format: "%.2f", arguments: [(((kill ?? 0) == 0) ? 0 : (kill ?? 0).float / ((matchs ?? 0) - (wins ?? 0)).float)])
    }
    
    fileprivate func killsPerMatches(kill: Int?, matchs: Int?) -> String {
